...
Use `scripts/generate_build_matrix.py` to expand every Slot/Module combination into ready-to-paste `build.yaml` entries:

```sh
python scripts/generate_build_matrix.py > build.generated.yaml
```

Pass `--dry-run` to inspect discovered modules or `--limit N` while experimenting. The output already terminates with the `settings_reset` target so it can fully replace the existing `build.yaml` when necessary.
